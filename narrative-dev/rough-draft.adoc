//.TODO
// - AsciidoctorJ and Asciidoctor.js sections need revising
...
* The mission of Asciidoctor
* A little surprise
* New features, capabilities, and performance of the core language and processor
* The growth and reach of Asciidoctor's ecosystem
* Where Asciidoctor is headed in the immediate future
* And finally, another little surprise

First off, let's remember what Asciidoctor is all about.
...
And we want readers to love reading documentation and learn effectively from it.
...
Asciidoctor is the leading implementation of AsciiDoc and the official home of the AsciiDoc language.
...
Having upwards of 250 comments, the issue is bursting at the seams.
After many different ideas, directions, and discussions, it's time to bring this thing to a close.
I'm proud to announce Asciidoctor's new logo, which was rendered by Marián Hlaváč.
...
Further initiatives involving the brand will happen in the newly minted brand repository.

== Processor Improvements

As we get into this section, it's important to understand that Asciidoctor is the leading implementation of AsciiDoc and the official home of the AsciiDoc language.
...
* icon:slide[language grid] translations of all built-in attributes in 28 languages available from the repository
...
* icon:slide[code example] much better SVG support, including support for interactive (object) and inline SVGs
* icon:slide[code example] much of the driving factor for the improvements to SVG support came from Asciidoctor Diagram.
Diagrams are best rendered in SVG.
Enabled both interactive diagrams and tighter integration with CSS to style them.

We also continue to refine the AsciiDoc language to give you as much capability in as few characters as possible.
...
In 2012, Ryan Waldron and Jeremy McAnally started a Ruby implementation of AsciiDoc.
...
The challenge was to develop enough of the parser and HTML converter so it could be deployed on GitHub before they pulled the plug on AsciiDoc.py.
...
And that transition gave Asciidoctor tremendous credibility.
GitHub still keeps in sync with Asciidocotor releases and, overall, AsciiDoc rendering there is pretty nice.
...
* you write full extensions purely in Java or Groovy, including a Groovy DSL that resembles the native one in Ruby
* has a maturing API which allows the creation and access to lists and tables
...
Bespoke converter, which was use to create this presentation
...
Adoption is growing like crazy. Year after year, Asciidoctor picks up notable users, and this year is no different.
...
* Oracle and the Java EE leadership recently announced that the Java EE tutorial will be rewritten in AsciiDoc and processed by Asciidoctor.
...
While it's provides great raw material, it needs to be overhauled.
That's where Antora comes in.
We're going to drink our own beer and migrate asciidoctor.org to Antora.
