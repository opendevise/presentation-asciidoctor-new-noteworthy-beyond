...
Asciidoctor now much better SVG support, including support for interactive (object) and inline SVGs (embedded).
...
Users have helped us find all sorts of edge cases, many of which we have fixes straightaway.
...
GitHub still keeps in sync with Asciidoctor releases and, in general, the AsciiDoc rendering there is pretty nice.
...
Also, the faster Asciidoctor runs, the quicker writers can see a preview of the document in a local editor.
...
There's no risk of diverence though as they are all based on the same core Ruby code.
...
* it also enables you write full extensions purely in Java or Groovy, including a Groovy DSL that resembles the native one in Ruby
...
* 1.6.0 in progress; it's really just waiting on the 1.6.0 release of Asciidoctor, which we're going to talk about shortly.
...
* Asciidoctor.js is on par with core; no reduction functionality
* it also offers a porcelain API (core & extension), which abstracts away artifacts left behind by Opal
...
When we made the HTML for the EPUB3 converter, we experiented with how AsciiDoc mapped to a semantic HTML structure.
...
The needs of the processor are just very different from the needs of a validation.
...
I looked around and discovered textlint.
